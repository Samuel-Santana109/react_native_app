...
import { API_URL, API_EMAIL, API_PASSWORD,BASE_URL } from "@env";
...
const sendEmail = async (name, emailUser, message) => {
  try {   
    const body = `<b>Nome:</b> ${name} <br> <b>Email:</b> ${emailUser} <br> <b>Message:</b> ${message}`
    const response = await axios.get(`${BASE_URL}/phpmail/index.php?to=suporte@piantecheguariscono.site&subject=Supporto&body=${body}`);
    return response.data;
  } catch (error) {
    throw error;
  }
...
export { getArticles, getCategories, getPlants, getAllCategories, getPlantsHeart, sendEmail };
